...
import MultiSelectDropdown, { SelectOrg, SelectTryonType, SelectGender, MultiSelectDropdown2, SubCategoryDropdown } from "./MultiselectDropodown"
...
const MainRow = ({ datard, id, PrOrList, props, DispalyNameRef, DispalyNameErrorRef, DispalyNameOnChange }) => {
  ...
  return <>
    <tr key={id} id={`mainRow-${id}`} style={{ backgroundColor: currentselected.id === id ? `#f7f7f7` : '' }} onClick={() => {
      ...
      {/* <td>
              <SubCategoryDropdown handelOnchange={handelOnchange} pid={id} identifiers={`Td_Sub_Category`} rootstage={true} datard={datard} td_Sub_Category={datard.td_Sub_Category} />
          </td> */}
      <td>
        <table className="ContentInner">
          {
