...
export const SubCategoryDropdown = (props) => {
    const { handelOnchange, pid, identifiers, rootstage, id } = props

    const [selectedList, setSelectedList] = useState([])
    const [newOption, setNewOption] = useState('')

    // Default list (can grow dynamically)
    const categoryList = props.SubCategoryList || ["Men", "Women", "Home", "Uniform", "Ethnic"]

    const objName = "td_SubCategory"

    const options = categoryList.map(z => ({
        [objName]: z,
        code: z
    }))

    useEffect(() => {
        if (props.datard?.td_Sub_Category) {
            const selected = options.filter(opt => props.datard.td_Sub_Category.includes(opt[objName]))
            //const selected = props.datard?.td_Sub_Category
    ...
    const handleAddOption = (event) => {
        ...
        if (newOption.trim() && !categoryList.includes(newOption)) {
            const newObj = { [objName]: newOption, code: newOption }

            categoryList.push(newOption)
            categoryList.sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }))

            const updatedList = [...selectedList, newObj]
            setSelectedList(updatedList)
            setNewOption('')

            // optional callback if needed
            if (props.onAddSubCategory) {
                props.onAddSubCategory(newObj)
            }
    ...
    const handleRemoveItem = (item, event) => {
        ...
        const index = categoryList.indexOf(item[objName])
        if (index !== -1) {
            categoryList.splice(index, 1)
        }

        const updatedList = selectedList.filter(i => JSON.stringify(i[objName]) !== JSON.stringify(item[objName]))
    ...
    return (
        <div className="multiselect-demo">
            ...
            <div className='input-01'>
                ...
                />
                <button
                    className="Add_Option btn btn-sm btn-primary"
            ...
            <div className="card">
                <MultiSelect
                    ...
                    itemTemplate={(option) => (
                        <div className='text'>
                            <span>{option[objName]}</span>

                            {/* ✅ Remove Icon */}
                            <i
                                className="p-multiselect-token-icon pi pi-times-circle"
                                style={{ cursor: 'pointer', color: "#4338CA", marginLeft: '8px' }}
                                onMouseOver={(e) => (e.target.style.color = 'red')}
                                onMouseOut={(e) => (e.target.style.color = '#4338CA')}
                                onClick={(event) => handleRemoveItem(option, event)}
                            />
